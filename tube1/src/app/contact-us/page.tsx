...
const DataTablePage: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_LIVE_BASE_URL}contacts/all`,
        );
  ...
  const deleteAllContacts = async () => {
    try {
      // Send DELETE request to backend to delete all contacts
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_LIVE_BASE_URL}contacts/deleteAll`,
        {
          method: "POST",
        },
      );
